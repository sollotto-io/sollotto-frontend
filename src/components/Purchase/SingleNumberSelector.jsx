...
export default function SingleNumberSelector(props) {
  ...
  function stepUpClickHandler(numPos) {
    try {
      if (numPos === 5) {
        if (
          Number(document.querySelector(`#ticketNum_${numPos}`).value) === 26 ||
          document.querySelector(`#ticketNum_${numPos}`).value.length === 0
        ) {
        ...
        if (
          Number(document.querySelector(`#ticketNum_${numPos}`).value) === 69 ||
          document.querySelector(`#ticketNum_${numPos}`).value.length === 0
        ) {
          document.querySelector(`#ticketNum_${numPos}`).value = 1;
        } else {
      ...
      if (numPos === 5) {
        if (
          ...
          document.querySelector(`#ticketNum_${numPos}`).value =
            Number(document.querySelector(`#ticketNum_${numPos}`).value) + step;
        }
      } else {
        if (
          Number(document.querySelector(`#ticketNum_${numPos}`).value) === 69 ||
  ...
  function stepDownClickHandler(numPos) {
    try {
      if (
        Number(document.querySelector(`#ticketNum_${numPos}`).value) === 1 ||
        document.querySelector(`#ticketNum_${numPos}`).value.length === 0
      ) {
        if (numPos === 5) {
  ...
  return (
    <div className="singleNumberSelector" id={`singleNumberSelector_${props.numPos}`}>
      ...
      />
      <NumberInput
        ticketPos={props.ticketPos}
        numPos={props.numPos}
        ticketNumChangeHandler={ticketNumChangeHandler}
      />
